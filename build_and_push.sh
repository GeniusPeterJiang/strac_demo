...
if [ -z "$LAMBDA_FUNC" ]; then
    ...
    echo "   RDS_ENDPOINT=\$(cd terraform && terraform output -raw rds_proxy_endpoint | cut -d: -f1)"
    echo "   psql -h \$RDS_ENDPOINT -U strac_admin -d scanner_db -f database_schema.sql"
    echo ""
    echo "3. Test the API:"
